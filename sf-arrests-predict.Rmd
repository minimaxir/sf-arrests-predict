...
plot <- ggplot(df_corr, aes(x=label1, y=label2, fill=value)) +
          ...
          scale_y_discrete() +
          coord_fixed() +
          theme(legend.title = element_text(size=7, family="Open Sans Condensed Bold"), legend.position="top", legend.direction="horizontal", legend.key.width=unit(1.75, "cm"), legend.key.height=unit(0.25, "cm"), legend.margin=unit(0,"cm"), panel.margin=element_blank(), axis.text.x=element_text(angle=-90), axis.title.y=element_blank(), axis.title.x=element_blank(), plot.title=element_text(hjust=1)) +
            scale_fill_gradient2(high = "#2ecc71", low = "#e74c3c", mid = "white", 
   midpoint = 0, limit = c(-0.5,0.5), 
   name="Pearson\nCorrelation", breaks=pretty_breaks(8))  +
            labs(title = "Correlations between Predicted Multiclass Probabilities of 63,144 SFPD Arrest Category Labels")
...
Predict arrest types at each point on March 14th, 2017, at 8 PM.
...
```{r}
date_target <- as.POSIXct("2017-04-15 23:00:00")

df_points <- df_points %>%
              mutate(month = format(date_target, "%m"),
                    hour = format(date_target, "%H"),
...
plot <- ggmap(map) +
            ...
            theme(legend.title = element_text(size=7, family="Open Sans Condensed Bold"), legend.position="top", legend.direction="horizontal", legend.key.width=unit(0.8, "cm"), legend.key.height=unit(0.25, "cm"), legend.margin=unit(0,"cm"), plot.title=element_text(hjust=0.5)) +
            labs(title = sprintf("Locations of Predicted Types of Arrests on %s",
