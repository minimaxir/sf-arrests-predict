...
# http://stackoverflow.com/questions/35709524/how-to-make-r-package-xgboost-parallel-in-os-x-with-openmp-compilation
#
# Install locally after compile via:
...
bst <- xgboost(data = train, label = df$category_index, nrounds = 2, subsample = 0.5, objective = "multi:softprob", eval_metric="mlogloss", num_class = length(table((df$category_index))))
```
...
```{r}
xgb.plot.multi.trees(model = bst, feature_names = colnames(train), features_keep = 3)
```
