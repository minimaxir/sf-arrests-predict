...
Use LightGBM's categorical data feature.

```{r}
train <- lgb.Dataset(df %>% select(X, Y, hour, month, year, DayOfWeek) %>% data.matrix(),
                     colnames = list("X", "Y", "hour", "month", "year", "DayOfWeek"),
                     categorical_feature = list("hour", "month", "DayOfWeek"),
                     label = df$category_index, free_raw_data=F)
num_classes <- length(unique(df$category_index))

bst <- lightgbm(data = train, nrounds = 200, num_threads=4, objective = "multiclass", metric="multi_logloss", num_class=num_classes, early_stopping_rounds = 3, nfolds=5, verbose = 1)
```
...
```{r}
preds_matrix <- matrix(predict(bst, train), byrow=T, num_rows, num_classes)
